          ...
          {TEMPLATES.map((t) => (
            ...
            >
              ...
              <p style={{ marginTop: 6, fontSize: 11 }}>
                {t.defaultWidth}×{t.defaultHeight}, оппонентов: {t.recommendedOpponents.min}–{t.recommendedOpponents.max}
              </p>
            </div>
          ))}
        </div>
      ...
      <div className="section">
        ...
        <div className="form-row">
          ...
          <input
            ...
            value={opponents}
            onChange={(e) => setOpponents(Math.max(tmpl.recommendedOpponents.min, Math.min(tmpl.recommendedOpponents.max, +e.target.value)))}
          />
        </div>
        <div className="form-row">
