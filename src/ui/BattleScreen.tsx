...
import {
  BATTLE_W, BATTLE_H, activeStack, canShoot, reachable, approachTiles,
  doMove, doAttack, doShoot, doWait, doDefend, isBattleOver, stepBattleAI, chebyshev,
...
export function BattleScreen() {
  ...
  useEffect(() => {
    ...
    if (act.side === 'defender') {
      if (!battle.defenderHeroId) isAi = true; // нейтрал
      else {
    ...
    <div className="battle-screen">
      ...
      <div className="battle-controls">
        {winner ? (
          ...
          <>
            <div style={{ fontSize: 14 }}>
              <span style={{ color: act.side === 'attacker' ? '#5fa850' : '#c44030' }}>●</span>{' '}
              Раунд {battle.round}: ход {UNITS[act.unitId].name} ({act.count})
            </div>
        ...
        <div className="battle-log">
          {battle.log.slice(-8).map((l, i) => (<div key={i}>{l}</div>))}
        </div>
      </div>
    </div>
  );
...
function drawBattle(ctx: CanvasRenderingContext2D, battle: ReturnType<typeof useGame.getState>['battle'], hover: Coord | null) {
  if (!battle) return;
