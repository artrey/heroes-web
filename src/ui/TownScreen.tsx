...
import { RESOURCE_ICONS, canAfford, RESOURCE_NAMES } from '../game/utils/resources';
  ...
  return (
    <div className="town-screen">
      <div className="top-bar">
        <span className="day">{town.faction === 'castle' ? '🏰' : '🏯'} {town.name}</span>
            ...
            {buildings.map((b) => {
              ...
              return (
                <div
                  ...
                  title={!prereqsOk ? `Требуется: ${b.prereq?.map((p) => getBuilding(town.faction, p)?.name).join(', ')}` : undefined}
                >
                  ...
                  {!built && (
                    <div className="cost">
                      {Object.entries(b.cost).map(([k, v]) => (
                        <span key={k}>{RESOURCE_ICONS[k as keyof ResourceBag]} {v}</span>
                      ))}
                    </div>
                  )}
                </div>
        ...
        <div className="town-sidebar">
            ...
            return (
              <div className="recruit-card" key={b.id}>
                ...
                <div style={{ display: 'flex', gap: 4, marginTop: 6 }}>
                  <button disabled={avail < 1 || !canBuyOne} onClick={() => hireUnits(town.id, unitId, 1)} style={{ flex: 1 }}>×1</button>
                  <button disabled={avail < 1 || !canBuyOne} onClick={() => hireUnits(town.id, unitId, avail)} style={{ flex: 1 }}>×{avail}</button>
                </div>
              </div>
            );
          })}
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {Array.from({ length: 7 }).map((_, idx) => {
              ...
              if (!stack) return <div key={idx} className="army-slot empty" style={{ flex: 1 }}>—</div>;
